#ifndef DBASE_STL_MEMORY_H_
#define DBASE_STL_MEMORY_H_
...
class temporary_buffer
{
    ...
    public:
        ...
        ~temporary_buffer()
        {
            dbase::stl::destroy(buffer, buffer + len);  // 析构
            free(buffer);
        }
    ...
    private:
        ...
        { dbase::stl::uninitialized_fill_n(buffer, len, value); }
        ...
        temporary_buffer(ForwardIterator first, ForwardIterator last)
{
    try
    {
        len = dbase::stl::distance(first, last);
        allocate_buffer();
        if (len > 0)
        {
