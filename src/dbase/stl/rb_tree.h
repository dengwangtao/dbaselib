#ifndef DBASE_STL_RB_TREE_H_
#define DBASE_STL_RB_TREE_H_
...
template <class T>
struct rb_tree_iterator_base : public dbase::stl::iterator<dbase::stl::bidirectional_iterator_tag, T>
{
...
NodePtr rb_tree_erase_rebalance(NodePtr z, NodePtr& root, NodePtr& leftmost, NodePtr& rightmost)
{
    ...
    if (y != z)
    {
        ...
        if (root == z)
            ...
            z->parent->right = y;
        y->parent = z->parent;
        dbase::stl::swap(y->color, z->color);
        y = z;
    }
    // y == z 说明 z 至多只有一个孩子
    else
    ...
    {  // x 为黑色时，调整，否则直接将 x 变为黑色即可
        while (x != root && (x == nullptr || !rb_tree_is_red(x)))
        {
            ...
            {  // 如果 x 为左子节点
                ...
                if ((brother->left == nullptr || !rb_tree_is_red(brother->left)) && (brother->right == nullptr || !rb_tree_is_red(brother->right)))
                {  // case 2
...
class rb_tree
{
    public:
        ...
        typedef dbase::stl::allocator<base_type> base_allocator;
    ...
    public:
        ...
        template <class... Args>
        dbase::stl::pair<iterator, bool> emplace_unique(Args&&... args);
        ...
        {
            return emplace_multi_use_hint(hint, dbase::stl::move(value));
        }
        ...
        dbase::stl::pair<iterator, bool> insert_unique(const value_type& value);
        dbase::stl::pair<iterator, bool> insert_unique(value_type&& value)
        {
        ...
        {
            return emplace_unique_use_hint(hint, dbase::stl::move(value));
        }
        ...
        size_type count_multi(const key_type& key) const
        {
            ...
            return static_cast<size_type>(dbase::stl::distance(p.first, p.second));
        }
        ...
        dbase::stl::pair<iterator, iterator>
        equal_range_multi(const key_type& key)
        {
            return dbase::stl::pair<iterator, iterator>(lower_bound(key), upper_bound(key));
        }
        dbase::stl::pair<const_iterator, const_iterator>
        equal_range_multi(const key_type& key) const
        {
            return dbase::stl::pair<const_iterator, const_iterator>(lower_bound(key), upper_bound(key));
        }
        ...
        equal_range_unique(const key_type& key)
        {
            ...
            auto next = it;
            return it == end() ? dbase::stl::make_pair(it, it) : dbase::stl::make_pair(it, ++next);
        }
        dbase::stl::pair<const_iterator, const_iterator>
    ...
    private:
        ...
        // get insert pos
        dbase::stl::pair<base_ptr, bool>
        get_insert_multi_pos(const key_type& key);
        dbase::stl::pair<dbase::stl::pair<base_ptr, bool>, bool>
...
rb_tree<T, Compare>::
        rb_tree(rb_tree&& rhs) noexcept
    : header_(dbase::stl::move(rhs.header_)),
      node_count_(rhs.node_count_),
      key_comp_(rhs.key_comp_)
{
    rhs.reset();
        ...
        emplace_multi(Args&&... args)
{
    ...
    node_ptr np = create_node(dbase::stl::forward<Args>(args)...);
...
template <class T, class Compare>
template <class... Args>
dbase::stl::pair<typename rb_tree<T, Compare>::iterator, bool>
rb_tree<T, Compare>::
        emplace_unique(Args&&... args)
{
    ...
    if (res.second)
    {  // 插入成功
        return dbase::stl::make_pair(insert_node_at(res.first.first, np, res.first.second), true);
    }
    destroy_node(np);
    return dbase::stl::make_pair(iterator(res.first.first), false);
}
        ...
        insert_unique(const value_type& value)
{
    ...
    if (res.second)
    {  // 插入成功
        return dbase::stl::make_pair(insert_value_at(res.first.first, value, res.first.second), true);
    }
    return dbase::stl::make_pair(res.first.first, false);
}
...
{
    auto p = equal_range_multi(key);
    size_type n = dbase::stl::distance(p.first, p.second);
        ...
        swap(rb_tree& rhs) noexcept
{
    if (this != &rhs)
    {
        dbase::stl::swap(header_, rhs.header_);
        dbase::stl::swap(node_count_, rhs.node_count_);
        dbase::stl::swap(key_comp_, rhs.key_comp_);
    }
}
...
template <class T, class Compare>
dbase::stl::pair<typename rb_tree<T, Compare>::base_ptr, bool>
rb_tree<T, Compare>::get_insert_multi_pos(const key_type& key)
{
    ...
    return dbase::stl::make_pair(y, add_to_left);
}

// get_insert_unique_pos 函数
template <class T, class Compare>
dbase::stl::pair<dbase::stl::pair<typename rb_tree<T, Compare>::base_ptr, bool>, bool>
...
{  // 返回一个 pair，第一个值为一个 pair，包含插入点的父节点和一个 bool 表示是否在左边插入，
    ...
    if (add_to_left)
    {
        ...
        {  // 如果树为空树或插入点在最左节点处，肯定可以插入新的节点
            return dbase::stl::make_pair(dbase::stl::make_pair(y, true), true);
        }
        else
    ...
    {  // 表明新节点没有重复
        return dbase::stl::make_pair(dbase::stl::make_pair(y, add_to_left), true);
    }
    // 进行至此，表示新节点与现有节点键值重复
    return dbase::stl::make_pair(dbase::stl::make_pair(y, add_to_left), false);
}
