#ifndef DBASE_STL_LIST_H_
#define DBASE_STL_LIST_H_
...
//
// 异常保证：
// dbase::stl::list<T> 满足基本异常保证，部分函数无异常保证，并对以下等函数做强异常安全保证：
//   * emplace_front
//   * emplace_back
//   * emplace
//   * push_front
...
// list 的迭代器设计
template <class T>
struct list_iterator : public dbase::stl::iterator<dbase::stl::bidirectional_iterator_tag, T>
{
...
class list
{
    public:
        ...
        typedef dbase::stl::allocator<list_node_base<T>> base_allocator;
        typedef dbase::stl::allocator<list_node<T>> node_allocator;
    ...
    public:
        ...
        void emplace_front(Args&&... args)
        {
            ...
            auto link_node = create_node(dbase::stl::forward<Args>(args)...);
        ...
        {
            THROW_LENGTH_ERROR_IF(size_ > max_size() - 1, "list<T>'s size too big");
            auto link_node = create_node(dbase::stl::move(value));
            ++size_;
            return link_iter_node(pos, link_node->as_base());
        }
        ...
        {
            dbase::stl::swap(node_, rhs.node_);
        ...
        void unique()
        { unique(dbase::stl::equal_to<T>()); }
        template <class BinaryPredicate>
        void unique(BinaryPredicate pred);

        void merge(list& x)
        { merge(x, dbase::stl::less<T>()); }
        template <class Compare>
        void merge(list& x, Compare comp);

        void sort()
        { list_sort(begin(), end(), size(), dbase::stl::less<T>()); }
...
void list<T>::reverse()
{
    ...
    {
        dbase::stl::swap(i.node_->prev, i.node_->next);
        i.node_ = i.node_->prev;
    }
    dbase::stl::swap(e.node_->prev, e.node_->next);
}
...
list<T>::create_node(Args&&... args)
{
    ...
    try
    {
        data_allocator::construct(dbase::stl::address_of(p->value), dbase::stl::forward<Args>(args)...);
        p->prev = nullptr;
        p->next = nullptr;
    }
    catch (...)
...
{
    data_allocator::destroy(dbase::stl::address_of(p->value));
    node_allocator::deallocate(p);
}
...
bool operator<(const list<T>& lhs, const list<T>& rhs)
{
    return dbase::stl::lexicographical_compare(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend());
}
