#ifndef DBASE_STL_DEQUE_H_
#define DBASE_STL_DEQUE_H_
...
//
// 异常保证：
// dbase::stl::deque<T> 满足基本异常保证，部分函数无异常保证，并对以下等函数做强异常安全保证：
//   * emplace_front
//   * emplace_back
//   * emplace
//   * push_front
...
class deque
{
    public:
        // deque 的型别定义
        typedef dbase::stl::allocator<T> allocator_type;
        typedef dbase::stl::allocator<T> data_allocator;
        typedef dbase::stl::allocator<T*> map_allocator;
        ...
        typedef deque_iterator<T, T&, T*> iterator;
        typedef deque_iterator<T, const T&, const T*> const_iterator;
        typedef dbase::stl::reverse_iterator<iterator> reverse_iterator;
        typedef dbase::stl::reverse_iterator<const_iterator> const_reverse_iterator;
    ...
    public:
        ...
        template <class IIter, typename std::enable_if<
                                       dbase::stl::is_input_iterator<IIter>::value, s32>::type = 0>
        deque(IIter first, IIter last)
        { copy_init(first, last, iterator_category(first)); }

        deque(std::initializer_list<value_type> ilist)
        {
            copy_init(ilist.begin(), ilist.end(), dbase::stl::forward_iterator_tag());
        }

        deque(const deque& rhs)
        {
            copy_init(rhs.begin(), rhs.end(), dbase::stl::forward_iterator_tag());
        }
        deque(deque&& rhs) noexcept
            : begin_(dbase::stl::move(rhs.begin_)),
              end_(dbase::stl::move(rhs.end_)),
              map_(rhs.map_),
    ...
    public:
        ...
        { copy_assign(ilist.begin(), ilist.end(), dbase::stl::forward_iterator_tag{}); }
        ...
        void push_front(value_type&& value) { emplace_front(dbase::stl::move(value)); }
        void push_back(value_type&& value) { emplace_back(dbase::stl::move(value)); }
...
deque<T>& deque<T>::operator=(const deque& rhs)
{
    if (this != &rhs)
    {
        ...
        if (len >= rhs.size())
        {
            erase(dbase::stl::copy(rhs.begin_, rhs.end_, begin_), end_);
        }
        else
        {
            iterator mid = rhs.begin() + static_cast<difference_type>(len);
            dbase::stl::copy(rhs.begin_, mid, begin_);
...
void deque<T>::emplace_front(Args&&... args)
{
    ...
    {
        data_allocator::construct(begin_.cur - 1, dbase::stl::forward<Args>(args)...);
        --begin_.cur;
    }
    else
    {
        ...
        {
            --begin_;
            data_allocator::construct(begin_.cur, dbase::stl::forward<Args>(args)...);
        }
        catch (...)
        {
            ++begin_;
...
void deque<T>::emplace_back(Args&&... args)
{
    ...
    {
        data_allocator::construct(end_.cur, dbase::stl::forward<Args>(args)...);
        ++end_.cur;
    }
    else
    {
...
typename deque<T>::iterator deque<T>::emplace(iterator pos, Args&&... args)
{
    ...
    {
        emplace_front(dbase::stl::forward<Args>(args)...);
        return begin_;
    }
    else if (pos.cur == end_.cur)
    {
        emplace_back(dbase::stl::forward<Args>(args)...);
        return end_ - 1;
    }
    return insert_aux(pos, dbase::stl::forward<Args>(args)...);
}
...
deque<T>::insert(iterator position, value_type&& value)
{
    if (position.cur == begin_.cur)
    {
        emplace_front(dbase::stl::move(value));
        return begin_;
    }
    else if (position.cur == end_.cur)
    {
        emplace_back(dbase::stl::move(value));
        auto tmp = end_;
        --tmp;
        return tmp;
    }
    ...
    {
        return insert_aux(position, dbase::stl::move(value));
    }
}
...
void deque<T>::insert(iterator position, size_type n, const value_type& value)
{
    if (position.cur == begin_.cur)
    {
        ...
        dbase::stl::uninitialized_fill_n(new_begin, n, value);
        begin_ = new_begin;
    }
    else if (position.cur == end_.cur)
    {
        ...
        auto new_end = end_ + n;
        dbase::stl::uninitialized_fill_n(end_, n, value);
        end_ = new_end;
    }
    else
    {
...
deque<T>::erase(iterator position)
{
    ...
    {
        dbase::stl::copy_backward(begin_, position, next);
        pop_front();
    }
    else
    {
        dbase::stl::copy(next, end_, position);
        pop_back();
    }
...
deque<T>::erase(iterator first, iterator last)
{
    ...
    else
    {
        ...
        {
            dbase::stl::copy_backward(begin_, first, last);
        ...
        {
            dbase::stl::copy(last, end_, first);
...
void deque<T>::clear()
{
    ...
    {  // 有两个以上的缓冲区
        dbase::stl::destroy(begin_.cur, begin_.last);
        dbase::stl::destroy(end_.first, end_.cur);
    }
    else
    {
        dbase::stl::destroy(begin_.cur, end_.cur);
    }
    shrink_to_fit();
    end_ = begin_;
}
...
void deque<T>::swap(deque& rhs) noexcept
{
    if (this != &rhs)
    {
        dbase::stl::swap(begin_, rhs.begin_);
        dbase::stl::swap(end_, rhs.end_);
        dbase::stl::swap(map_, rhs.map_);
        dbase::stl::swap(map_size_, rhs.map_size_);
    }
}
        ...
        map_init(size_type nElem)
{
    ...
    map_size_ = dbase::stl::max(static_cast<size_type>(DEQUE_MAP_INIT_SIZE), nNode + 2);
    try
    {
        ...
        fill_init(size_type n, const value_type& value)
{
    ...
    if (n != 0)
    {
        ...
        {
            dbase::stl::uninitialized_fill(*cur, *cur + buffer_size, value);
        }
        dbase::stl::uninitialized_fill(end_.first, end_.cur, value);
    }
}
...
{
    const size_type n = dbase::stl::distance(first, last);
    map_init(n);
        ...
        copy_init(FIter first, FIter last, forward_iterator_tag)
{
    ...
    {
        auto next = first;
        dbase::stl::advance(next, buffer_size);
        dbase::stl::uninitialized_copy(first, next, *cur);
        first = next;
    }
    dbase::stl::uninitialized_copy(first, last, end_.first);
}
        ...
        fill_assign(size_type n, const value_type& value)
{
    if (n > size())
    {
        dbase::stl::fill(begin(), end(), value);
        ...
        copy_assign(FIter first, FIter last, forward_iterator_tag)
{
    ...
    const size_type len2 = dbase::stl::distance(first, last);
    if (len1 < len2)
    {
        auto next = first;
        dbase::stl::advance(next, len1);
        dbase::stl::copy(first, next, begin_);
        insert_dispatch(end_, next, last, forward_iterator_tag{});
    }
    else
    {
        erase(dbase::stl::copy(first, last, begin_), end_);
    }
}
...
{
    const size_type elems_before = position - begin_;
    value_type value_copy = value_type(dbase::stl::forward<Args>(args)...);
    if (elems_before < (size() / 2))
    {  // 在前半段插入
        emplace_front(front());
        auto front1 = begin_;
        ...
        auto pos = position;
        ++pos;
        dbase::stl::copy(front2, pos, front1);
    }
    else
    {  // 在后半段插入
        emplace_back(back());
        ...
        --back2;
        position = begin_ + elems_before;
        dbase::stl::copy_backward(position, back2, back1);
    }
    *position = dbase::stl::move(value_copy);
    return position;
}
        ...
        fill_insert(iterator position, size_type n, const value_type& value)
{
    ...
    if (elems_before < (len / 2))
    {
        ...
        try
        {
            if (elems_before >= n)
            {
                ...
                dbase::stl::uninitialized_copy(begin_, begin_n, new_begin);
                begin_ = new_begin;
                dbase::stl::copy(begin_n, position, old_begin);
                dbase::stl::fill(position - n, position, value_copy);
            }
            else
            {
                dbase::stl::uninitialized_fill(
                        dbase::stl::uninitialized_copy(begin_, position, new_begin), begin_, value_copy);
                begin_ = new_begin;
                dbase::stl::fill(old_begin, position, value_copy);
            }
        }
        catch (...)
        {
    ...
    else
    {
        ...
        try
        {
            ...
            {
                auto end_n = end_ - n;
                dbase::stl::uninitialized_copy(end_n, end_, end_);
                end_ = new_end;
                dbase::stl::copy_backward(position, end_n, old_end);
                dbase::stl::fill(position, position + n, value_copy);
            }
            else
            {
                dbase::stl::uninitialized_fill(end_, position + n, value_copy);
                dbase::stl::uninitialized_copy(position, end_, position + n);
                end_ = new_end;
                dbase::stl::fill(position, old_end, value_copy);
            }
        }
        catch (...)
        {
    ...
    if (elems_before < (len / 2))
    {
        ...
        try
        {
            if (elems_before >= n)
            {
                ...
                begin_ = new_begin;
                dbase::stl::copy(begin_n, position, old_begin);
                dbase::stl::copy(first, last, position - n);
            }
            else
            {
                auto mid = first;
                dbase::stl::advance(mid, n - elems_before);
                dbase::stl::uninitialized_copy(first, mid,
                                               dbase::stl::uninitialized_copy(begin_, position, new_begin));
                begin_ = new_begin;
                dbase::stl::copy(mid, last, old_begin);
            }
        }
        catch (...)
        {
    ...
    else
    {
        ...
        try
        {
            if (elems_after > n)
            {
                ...
                end_ = new_end;
                dbase::stl::copy_backward(position, end_n, old_end);
                dbase::stl::copy(first, last, position);
            }
            else
            {
                auto mid = first;
                dbase::stl::advance(mid, elems_after);
                dbase::stl::uninitialized_copy(position, end_,
                                               dbase::stl::uninitialized_copy(mid, last, end_));
                end_ = new_end;
                dbase::stl::copy(first, mid, position);
            }
        }
        catch (...)
        {
        ...
        insert_dispatch(iterator position, FIter first, FIter last, forward_iterator_tag)
{
    ...
    if (position.cur == begin_.cur)
    {
        ...
        try
        {
            dbase::stl::uninitialized_copy(first, last, new_begin);
            begin_ = new_begin;
        }
        catch (...)
        {
    ...
    else if (position.cur == end_.cur)
    {
        ...
        try
        {
            dbase::stl::uninitialized_copy(first, last, end_);
            end_ = new_end;
        }
        catch (...)
        {
...
{
    const size_type new_map_size = dbase::stl::max(map_size_ << 1,
...
{
    return lhs.size() == rhs.size() && dbase::stl::equal(lhs.begin(), lhs.end(), rhs.begin());
}

template <class T>
bool operator<(const deque<T>& lhs, const deque<T>& rhs)
{
    return dbase::stl::lexicographical_compare(
