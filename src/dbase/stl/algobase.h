#ifndef DBASE_STL_ALGOBASE_H_
#define DBASE_STL_ALGOBASE_H_

// 这个头文件包含了 dbase::stl 的基本算法
...
OutputIter unchecked_copy_impl(RandomIter first, RandomIter last, OutputIter result,
                               dbase::stl::random_access_iterator_tag)
{
...
unchecked_copy_backward_cat(BidirectionalIter1 first, BidirectionalIter1 last,
                            BidirectionalIter2 result, dbase::stl::bidirectional_iterator_tag)
{
    while (first != last)
        *--result = *--last;
    return result;
...
unchecked_copy_backward_cat(RandomIter1 first, RandomIter1 last,
                            BidirectionalIter2 result, dbase::stl::random_access_iterator_tag)
{
...
typename std::enable_if<
        dbase::stl::is_same_v<typename dbase::stl::remove_const_t<Tp>, Up> && std::is_trivially_copy_assignable<Up>::value,
        Up*>::type
...
dbase::stl::pair<InputIter, OutputIter>
unchecked_copy_n(InputIter first, Size n, OutputIter result, dbase::stl::input_iterator_tag)
{
    for (; n > 0; --n, ++first, ++result)
    {
        *result = *first;
    }
    return dbase::stl::pair<InputIter, OutputIter>(first, result);
}

template <class RandomIter, class Size, class OutputIter>
dbase::stl::pair<RandomIter, OutputIter>
unchecked_copy_n(RandomIter first, Size n, OutputIter result,
                 dbase::stl::random_access_iterator_tag)
{
    auto last = first + n;
    return dbase::stl::pair<RandomIter, OutputIter>(last, dbase::stl::copy(first, last, result));
}
...
template <class Tp, class Up>
typename std::enable_if<
        std::is_same<typename std::remove_const<Tp>::type, Up>::value && std::is_trivially_move_assignable<Up>::value,
        Up*>::type
...
unchecked_move_backward_cat(RandomIter1 first, RandomIter1 last,
                            RandomIter2 result, dbase::stl::random_access_iterator_tag)
{
    for (auto n = last - first; n > 0; --n)
        *--result = dbase::stl::move(*--last);
    return result;
...
typename std::enable_if<
        std::is_integral<Tp>::value && sizeof(Tp) == 1 && !std::is_same<Tp, bool>::value && std::is_integral<Up>::value && sizeof(Up) == 1,
        Tp*>::type
...
void fill_cat(ForwardIter first, ForwardIter last, const T& value,
              dbase::stl::forward_iterator_tag)
{
    for (; first != last; ++first)
    {
        *first = value;
                             ...
                             const unsigned char* last2)
{
    ...
    // 先比较相同长度的部分
    const auto result = std::memcmp(first1, first2, dbase::stl::min(len1, len2));
    // 若相等，长度较长的比较大
...
/*****************************************************************************************/
template <class InputIter1, class InputIter2>
dbase::stl::pair<InputIter1, InputIter2>
mismatch(InputIter1 first1, InputIter1 last1, InputIter2 first2)
{
    ...
    return dbase::stl::pair<InputIter1, InputIter2>(first1, first2);
}
