#ifndef DBASE_STL_BASIC_STRING_H_
#define DBASE_STL_BASIC_STRING_H_
...
// 模板类 basic_string
// 参数一代表字符类型，参数二代表萃取字符类型的方式，缺省使用 dbase::stl::char_traits
template <class CharType, class CharTraits = dbase::stl::char_traits<CharType>>
class basic_string
{
    public:
        ...
        typedef dbase::stl::allocator<CharType> allocator_type;
        typedef dbase::stl::allocator<CharType> data_allocator;
    ...
    public:
        ...
        template <class Iter, typename std::enable_if<
                                      dbase::stl::is_input_iterator_v<Iter>, s32>::type = 0>
    ...
    public:
        ...
        {
            count = dbase::stl::min(count, size_ - index);
    ...
    private:
        ...
        template <class Iter>
        void copy_init(Iter first, Iter last, dbase::stl::input_iterator_tag);
        template <class Iter>
        void copy_init(Iter first, Iter last, dbase::stl::forward_iterator_tag);
        ...
        insert(const_iterator pos, Iter first, Iter last)
{
    ...
    if (pos == end())
    {
        dbase::stl::uninitialized_copy(first, last, end());
        size_ += len;
        return r;
    }
    char_traits::move(r + len, r, len);
    dbase::stl::uninitialized_copy(first, last, r);
    size_ += len;
    return r;
}
...
{
    auto n1 = dbase::stl::min(count1, size_ - pos1);
...
{
    auto n1 = dbase::stl::min(count1, size_ - pos1);
    auto n2 = dbase::stl::min(count2, other.size_ - pos2);
        ...
        swap(basic_string& rhs) noexcept
{
    if (this != &rhs)
    {
        dbase::stl::swap(buffer_, rhs.buffer_);
...
{
    const auto init_size = dbase::stl::max(static_cast<size_type>(STRING_INIT_SIZE), n + 1);
...
template <class Iter>
void basic_string<CharType, CharTraits>::
        copy_init(Iter first, Iter last, dbase::stl::input_iterator_tag)
{
...
template <class Iter>
void basic_string<CharType, CharTraits>::
        copy_init(Iter first, Iter last, dbase::stl::forward_iterator_tag)
{
    ...
    try
    {
        ...
        size_ = n;
        cap_ = init_size;
        dbase::stl::uninitialized_copy(first, last, buffer_);
    }
    catch (...)
    {
        buffer_ = nullptr;
...
{
    const auto init_size = dbase::stl::max(static_cast<size_type>(STRING_INIT_SIZE), count + 1);
    buffer_ = data_allocator::allocate(init_size);
        ...
        append_range(Iter first, Iter last)
{
    ...
    dbase::stl::uninitialized_copy_n(first, n, buffer_ + size_);
    size_ += n;
    return *this;
}
...
{
    const auto new_cap = dbase::stl::max(cap_ + need, cap_ + (cap_ >> 1));
        ...
        reallocate_and_fill(iterator pos, size_type n, value_type ch)
{
    ...
    const auto new_cap = dbase::stl::max(old_cap + n, old_cap + (old_cap >> 1));
    auto new_buffer = data_allocator::allocate(new_cap);
        ...
        reallocate_and_copy(iterator pos, const_iterator first, const_iterator last)
{
    ...
    auto new_buffer = data_allocator::allocate(new_cap);
    auto e1 = char_traits::move(new_buffer, buffer_, r) + r;
    auto e2 = dbase::stl::uninitialized_copy_n(first, n, e1) + n;
    char_traits::move(e2, buffer_ + r, size_ - r);
    data_allocator::deallocate(buffer_, old_cap);
    buffer_ = new_buffer;
    size_ += n;
...
operator+(basic_string<CharType, CharTraits>&& lhs,
          const basic_string<CharType, CharTraits>& rhs)
{
    basic_string<CharType, CharTraits> tmp(dbase::stl::move(lhs));
    tmp.append(rhs);
    return tmp;
}
...
{
    basic_string<CharType, CharTraits> tmp(dbase::stl::move(rhs));
...
#endif  // !DBASE_STL_BASIC_STRING_H_
