#ifndef DBASE_STL_QUEUE_H_
#define DBASE_STL_QUEUE_H_
...
// 模板类 queue
// 参数一代表数据类型，参数二代表底层容器类型，缺省使用 dbase::stl::deque 作为底层容器
template <class T, class Container = dbase::stl::deque<T>>
class queue
{
    public:
    ...
    public:
        ...
        { c_.emplace_back(dbase::stl::forward<Args>(args)...); }

        void push(const value_type& value)
        { c_.push_back(value); }
        void push(value_type&& value)
        { c_.emplace_back(dbase::stl::move(value)); }
        ...
        void swap(queue& rhs) noexcept(noexcept(dbase::stl::swap(c_, rhs.c_)))
        { dbase::stl::swap(c_, rhs.c_); }
...
// 模板类 priority_queue
// 参数一代表数据类型，参数二代表容器类型，缺省使用 dbase::stl::vector 作为底层容器
// 参数三代表比较权值的方式，缺省使用 dbase::stl::less 作为比较方式
template <class T, class Container = dbase::stl::vector<T>, class Compare = dbase::stl::less<typename Container::value_type>>
class priority_queue
{
    ...
    public:
        ...
        explicit priority_queue(size_type n)
            : c_(n)
        {
            dbase::stl::make_heap(c_.begin(), c_.end(), comp_);
        }
        ...
        priority_queue(priority_queue&& rhs)
            : c_(dbase::stl::move(rhs.c_)), comp_(rhs.comp_)
        {
            dbase::stl::make_heap(c_.begin(), c_.end(), comp_);
        }
    ...
    public:
        ...
        {
            c_.emplace_back(dbase::stl::forward<Args>(args)...);
            dbase::stl::push_heap(c_.begin(), c_.end(), comp_);
        }
        ...
        void push(value_type&& value)
        {
            c_.push_back(dbase::stl::move(value));
            dbase::stl::push_heap(c_.begin(), c_.end(), comp_);
        }

        void pop()
        {
            dbase::stl::pop_heap(c_.begin(), c_.end(), comp_);
            c_.pop_back();
        }
        ...
        void swap(priority_queue& rhs) noexcept(noexcept(dbase::stl::swap(c_, rhs.c_)) && noexcept(dbase::stl::swap(comp_, rhs.comp_)))
        {
            dbase::stl::swap(c_, rhs.c_);
            dbase::stl::swap(comp_, rhs.comp_);
        }
