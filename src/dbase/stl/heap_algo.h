#ifndef DBASE_STL_HEAP_ALGO_H_
#define DBASE_STL_HEAP_ALGO_H_
                 ...
                 Compared comp = Compared())
{
    ...
    *(first + holeIndex) = value;
    ...
    while (topIndex < len)
    {
        ...
        // 交换父节点和子节点
        dbase::stl::swap(*(first + oldTopIndex), *(first + topIndex));
    }
}
...
{
    dbase::stl::adjust_heap_to_up(first, last - first - 1, static_cast<Distance>(0), value, comp);
}
...
{
    // 新加入的元素在尾部
    dbase::stl::push_heap_aux(first, last, *(last - 1), distance_type(first), comp);
}
                  ...
                  T value, Distance*, Compared comp = Compared())
{
    *result = *first;  // 先将尾指设置成首值，即尾指为欲求结果
    dbase::stl::adjust_heap(first, static_cast<Distance>(0), last - first, value, comp);
}
...
{
    dbase::stl::pop_heap_aux(first, last - 1, last - 1, *(last - 1),
...
void sort_heap(RandomIter first, RandomIter last, Compared comp = Compared())
{
    while (last - first > 1)
    {
        dbase::stl::pop_heap(first, last--, comp);
    }
}
...
void make_heap_aux(RandomIter first, RandomIter last, Distance*, Compared comp)
{
    ...
    {
        // 重排以 holeIndex 为首的子树
        dbase::stl::adjust_heap(first, holeIndex, len, *(first + holeIndex), comp);
        if (holeIndex == 0)
        {
            return;
        }
...
{
    dbase::stl::make_heap_aux(first, last, distance_type(first), comp);
}

}  // namespace dbase::stl
#endif  // !DBASE_STL_HEAP_ALGO_H_
