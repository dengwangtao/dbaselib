#ifndef DBASE_STL_UTIL_H_
#define DBASE_STL_UTIL_H_
...
struct pair
{
        ...
        template <class U1 = Ty1, class U2 = Ty2, typename std::enable_if<std::is_copy_constructible<U1>::value && std::is_copy_constructible<U2>::value && std::is_convertible<const U1&, Ty1>::value && std::is_convertible<const U2&, Ty2>::value, s32>::type = 0>
        ...
        template <class Other1, class Other2, typename std::enable_if<std::is_constructible<Ty1, Other1>::value && std::is_constructible<Ty2, Other2>::value && std::is_convertible<Other1&&, Ty1>::value && std::is_convertible<Other2&&, Ty2>::value, s32>::type = 0>
        constexpr pair(Other1&& a, Other2&& b)
            : first(dbase::stl::forward<Other1>(a)),
              second(dbase::stl::forward<Other2>(b))
        {
        }
        ...
        template <class Other1, class Other2, typename std::enable_if<std::is_constructible<Ty1, const Other1&>::value && std::is_constructible<Ty2, const Other2&>::value && std::is_convertible<const Other1&, Ty1>::value && std::is_convertible<const Other2&, Ty2>::value, s32>::type = 0>
        ...
        constexpr pair(pair<Other1, Other2>&& other)
            : first(dbase::stl::forward<Other1>(other.first)),
              second(dbase::stl::forward<Other2>(other.second))
        {
        }
        ...
        pair& operator=(pair&& rhs)
        {
            if (this != &rhs)
            {
                first = dbase::stl::move(rhs.first);
                second = dbase::stl::move(rhs.second);
            }
            return *this;
        }
        ...
        void swap(pair& other)
        {
            if (this != &other)
            {
                dbase::stl::swap(first, other.first);
                dbase::stl::swap(second, other.second);
            }
        }
